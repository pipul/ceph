...
void OSDService::check_nearfull_warning(const osd_stat_t &osd_stat)
{
  ...
  if (cur_state != new_state) {
    cur_state = new_state;
  } else if (now - last_msg < g_conf->op_tracker_complaint_time) { //
    return;
  }
  last_msg = now;
  if (cur_state == FULL)
...
{
  MOSDSubOpReply *m = static_cast<MOSDSubOpReply*>(op->get_req());
...
{
  utime_t latency = ceph_clock_now(g_ceph_context) - op->get_req()->get_recv_stamp();
