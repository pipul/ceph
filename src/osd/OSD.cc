...
int OSD::init()
{
  ...
  int compat = osd_compat.compare(superblock.compat_features);
  if (compat == -1) {
    dout(0) << "Incompatible features found" << dendl;
    dout(0) << "OSD features: " << osd_compat << dendl;
    dout(0) << "On disk features: " << superblock.compat_features << dendl;
    return -EINVAL;
  }
